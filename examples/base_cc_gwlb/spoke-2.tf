...
module "spoke_2_network" {
  ...
  gwlb_endpoint_ids = module.spoke_2_gwlb_endpoint.gwlbe
}
...
module "spoke_2_bastion" {
  ...
  resource_tag              = random_string.suffix.result
  global_tags               = local.global_tags
  vpc_id                    = module.spoke_2_network.vpc_id
  public_subnet             = module.spoke_2_network.public_subnet_ids[0]
...
module "spoke_2_workload" {
  ...
  global_tags    = local.global_tags
  vpc_id         = module.spoke_2_network.vpc_id
  subnet_id      = module.spoke_2_network.workload_subnet_ids
  instance_key   = aws_key_pair.deployer.key_name
  workload_private_ip = var.workload_private_ip
  workload_iam_role_name = "spoke-2-iam-role-${random_string.suffix.result}"
}
...
################################################################################
module "spoke_2_gwlb_endpoint" {
  source              = "../../modules/terraform-zscc-spoke-gwlbendpoint-aws"
  name_prefix         = var.name_prefix
  resource_tag        = random_string.suffix.result
  global_tags         = local.global_tags
  vpc_id              = module.spoke_2_network.vpc_id
  subnet_ids          = module.spoke_2_network.workload_subnet_ids
