...
variable "hub_vpc_cidr" {
  type        = string
  description = "Hub VPC CIDR for CC/GWLB/TGW resources. All hub subnets (public, TGW attach, GWLB endpoint, CC) are derived from this /16 CIDR."
  default     = "10.0.0.0/16"
}

variable "spoke_1_vpc_cidr" {
  type        = string
  description = "Spoke 1 VPC CIDR for workload resources."
  default     = "10.1.0.0/16"
}

variable "spoke_2_vpc_cidr" {
  type        = string
  description = "Spoke 2 VPC CIDR for workload resources."
  default     = "10.2.0.0/16"
}

variable "tgw_name" {
  type        = string
  description = "Name for the Transit Gateway shared between Hub and Spoke VPCs."
  default     = "zscc-tgw"
}

variable "public_subnets" {
  type        = list(string)
  description = "Public/NAT GW Subnets to create in Hub VPC. Override the default subnets derived from hub_vpc_cidr."
  default     = null
}

variable "workloads_subnets" {
  type        = list(string)
  description = "Workload Subnets to create in Spoke VPCs. Override the default subnets derived from spoke VPC CIDRs."
  default     = null
}

variable "cc_subnets" {
  type        = list(string)
  description = "Cloud Connector Subnets to create in Hub VPC. Override the default subnets derived from hub_vpc_cidr."
  default     = null
}

variable "workload_count" {
  type        = number
  description = "Default number of workload VMs to create per spoke VPC"
  default     = 1
}

variable "workload_private_ip" {
  type        = string
  description = "Hard coded IP address for workload in spoke 1 VPC"
  default     = "10.1.1.50"
}

variable "workload_2_private_ip" {
  type        = string
  description = "Hard coded IP address for workload in spoke 2 VPC"
  default     = "10.2.1.50"
}
