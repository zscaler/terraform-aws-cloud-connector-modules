...
resource "aws_vpc" "spoke_2" {
  ...
  tags = merge(local.global_tags, { Name = "${var.name_prefix}-spoke-2-vpc-${random_string.suffix.result}" })
}

resource "aws_internet_gateway" "spoke_2_igw" {
  vpc_id = aws_vpc.spoke_2.id

  tags = merge(local.global_tags, { Name = "${var.name_prefix}-spoke-2-igw-${random_string.suffix.result}" })
}

data "aws_availability_zones" "spoke_2_available" {
  state = "available"
}
...
resource "aws_subnet" "spoke_2_public" {
  ...
  availability_zone       = data.aws_availability_zones.spoke_2_available.names[0]
  map_public_ip_on_launch = true

  tags = merge(local.global_tags, { Name = "${var.name_prefix}-spoke-2-public-subnet-${random_string.suffix.result}" })
}
...
resource "aws_subnet" "spoke_2_workload" {
  ...
  availability_zone = data.aws_availability_zones.spoke_2_available.names[count.index]

  tags = merge(local.global_tags, { Name = "${var.name_prefix}-spoke-2-workload-subnet-${count.index + 1}-${random_string.suffix.result}" })
}
...
resource "aws_route_table" "spoke_2_public_rt" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.spoke_2_igw.id
  }

  tags = merge(local.global_tags, { Name = "${var.name_prefix}-spoke-2-public-rt-${random_string.suffix.result}" })
}

resource "aws_route_table_association" "spoke_2_public_rta" {
  subnet_id      = aws_subnet.spoke_2_public.id
  route_table_id = aws_route_table.spoke_2_public_rt.id
}
...
resource "aws_route_table" "spoke_2_workload_rt" {
  ...
  tags = merge(local.global_tags, { Name = "${var.name_prefix}-spoke-2-workload-rt-${count.index + 1}-${random_string.suffix.result}" })
}

resource "aws_route_table_association" "spoke_2_workload_rta" {
  count          = var.az_count
...
################################################################################
# 2. Create Bastion Host for workload SSH jump access in Spoke 2 VPC
################################################################################
module "spoke_2_bastion" {
  source                    = "../../modules/terraform-zscc-bastion-aws"
  name_prefix               = "${var.name_prefix}-sp2"
  resource_tag              = random_string.suffix.result
  global_tags               = local.global_tags
  vpc_id                    = aws_vpc.spoke_2.id
  public_subnet             = aws_subnet.spoke_2_public.id
