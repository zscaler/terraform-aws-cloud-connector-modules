...
# 7. Create Lambda for HA between pair of Cloud Connectors.
module "cc-lambda" {
  ...
  cc_vm1_rte_list = [module.network.workload-route-table-ids[0]]
  cc_vm2_rte_list = [module.network.workload-route-table-ids[1]]
  http_probe_port = var.http_probe_port
}


# 8. Create Route 53 Resolver Rules and Endpoints for utilization with DNS redirection to facilitate Cloud Connector ZPA service
module "route53" {
