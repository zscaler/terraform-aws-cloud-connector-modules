...
variable "vpc_cidr" {
  type        = string
  description = "VPC IP CIDR Range. All subnet resources that might get created (public, workload, Zscaler) are derived from this /16 CIDR. If you require creating a VPC smaller than /16, you may need to explicitly define all other subnets via public_subnets, workload_subnets, cc_subnets, and route53_subnets variables"
  default     = "10.1.0.0/16"
}

variable "endpoint_subnets" {
  type        = list(string)
  description = "Zscaler Subnets to create in VPC. This is only required if you want to override the default subnets that this code creates via vpc_cidr variable."
  default     = null
}
...
variable "zs_route_table_enabled" {
  type        = bool
  description = "For brownfield environments where VPC subnets already exist, set to false to not create a new route table to associate to Zscaler subnet(s). Default is true which means module will try to create new route tables"
  default     = true
}
