...
# Or use existing IAM Instance Profile if specified in byo_iam_instance_profile
data "aws_iam_instance_profile" "cc-host-profile-selected" {
  count = var.byo_iam_instance_profile == false ? length(aws_iam_instance_profile.cc-host-profile.*.id) : length(var.byo_iam_instance_profile_id)
  name  = var.byo_iam_instance_profile == false ? element(aws_iam_instance_profile.cc-host-profile.*.name, count.index) : element(var.byo_iam_instance_profile_id, count.index)
}